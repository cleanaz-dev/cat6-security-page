...
export async function createPaymentLink({ uuid, email, amount = 500, redirectUrl }) {
  return await stripe.paymentLinks.create({
    ...
    metadata: { uuid, email }, // Just the essentials
    after_completion: {
      type: 'redirect',
      redirect: { url: `${redirectUrl}?payment=success&id=${uuid}` }
    },
    expires_at: Math.floor(Date.now() / 1000) + 86400 // 24h expiry
  });
}
...
export async function createCheckoutSession({ uuid, email, redirectUrl }) {
  const session = await stripe.checkout.sessions.create({
    payment_method_types: ['card'],
    customer_email: email,
    
    // Use existing price ID instead of price_data
    line_items: [{
      price: price_1RK40nA2zLkBaPCsfBvZcOZj, // e.g., 'price_1234567890'
      quantity: 1,
    }],
    
    mode: 'payment',
     success_url: `${redirectUrl}/deposit-confirmed?payment=success&id=${uuid}&session_id={CHECKOUT_SESSION_ID}`,
    cancel_url: `${redirectUrl}/deposit-cancelled?payment=cancelled&id=${uuid}`,
    metadata: { uuid, email },
