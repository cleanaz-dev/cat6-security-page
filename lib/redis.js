...
export async function getOpenTickets() {
  try {
    ...
    // console.log("ticket keys", keys)
    if (!keys || keys.length === 0) {
    console.warn("No tickets found")
    return []
    }
    const openTickets = await redis.json.mGet(keys, '$')
    return openTickets.flat().filter(ticket => ticket.status === "open")
  } catch (error) {
    console.error(error)
    throw error
  }
...
export async function getQuoteById(quoteId) {
  try {
    const quote = await redis.json.get(`quoteId:${quoteId}`)
    return quote
  } catch (error) {
    console.error(error)
    throw error
...
export async function getNotesByTicketId(ticketId){
  try {
    const notes = await redis.json.get(`ticket:${ticketId}`)
    if (!notes.notes) {
      return []
    }
...
export async function getInstallById(installId) {
  try {
    const install = await redis.json.get(`install:${installId}`)
    if (!install) {
      return null
    }
    return install
