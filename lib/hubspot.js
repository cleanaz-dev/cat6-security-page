...
export async function getAllActivityForContact(contactEmail) {
  try {
    ...
    const [callIds, emailIds, taskIds, dealIds] = await Promise.all([
    ...
    const [calls, emails, tasks, deals] = await Promise.all([
      fetchObjects('calls'),
      fetchObjects('emails'),
      fetchObjects('tasks'),
      fetchObjects('deals'),
    ...
    return { calls, emails, tasks, deals };
  ...
  try {
    const payload = {
      ...
      associations: [{
        ...
        types: [{
          associationCategory: "HUBSPOT_DEFINED",
          associationTypeId: 280, // Note-to-Contact association
        }]
      }]
    };
